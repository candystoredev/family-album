...
- **10.3 Historical backfill (separate track; runs on a machine without Claude)** —
  ...
  - **10.3b Tool B — Matcher/Applier** — phash match to stored thumbnails, confidence thresholds, applies to new columns only via an authed admin endpoint.
    - **Stale-place gotcha — must be handled when Tool B starts writing GPS.** `npm run backfill:geocode` only considers rows `WHERE … place IS NULL`, and its `UPDATE` is guarded `AND place IS NULL` (`scripts/backfill-geocode.ts`). So a row whose coordinates Tool B *corrects* (Apple-Photos GPS beating the EXIF read at upload) keeps its old, now-wrong place label forever — a re-run silently skips it. Nothing can produce this today (all 203 labels came from the same GPS the backfill read, 2026-07-21), but it goes live with Tool B. Fix either way: have Tool B `SET place = NULL` on any row whose `gps_lat`/`gps_lng` it changes, so the next geocode run re-derives it; or add a `--force` flag. Prefer the former — it keeps "geocode fills what's missing" as the single rule. Rows given GPS *for the first time* are already handled (they're `place IS NULL`, so a re-run picks them up).
    - Any run that changes `media.place` needs the FTS rebuild afterwards to become searchable; `backfill:geocode` does this itself at the end, so sequencing geocode last covers it.
