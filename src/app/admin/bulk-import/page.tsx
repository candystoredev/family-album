...
import { buildCaptureInput, sha256Hex, extractPhotoExtras } from "@/lib/media/extract";
    ...
    try {
      ...
      const uploadedItems = await Promise.all(
        groupItems.map(async (item) => {
          ...
          // Hash + extract from the original here (not at ingest) so we don't
          // read every original up front (10.1b/c).
          const contentHash = (await sha256Hex(item.file)) ?? undefined;
          const extras = await extractPhotoExtras(item.file);
          ...
          return { r2Key, keyPrefix, type: "photo" as const, capture: item.capture, contentHash, meta: extras };
        })
      );
